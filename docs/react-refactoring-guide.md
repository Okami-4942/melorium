...
現在の初期画面は、旧`main.js`の灰色のメイン部屋です。Forte・Mezzo・Pianoの三つのドアは表示と当たり判定のみ実装され、別の部屋へ移動する処理はまだありません。

`mezzo`・`forte`・`piano`の三部屋を設定データとして追加し、メイン部屋からドアで移動する機能は、学習課題として[`door-room-transition-guide.md`](./door-room-transition-guide.md)に実装手順をまとめています。
